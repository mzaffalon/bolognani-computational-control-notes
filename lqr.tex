...
\item the cost is addictive over time
  ...
  \end{equation*}
\end{itemize}
Under these assumptions, Bellman's principle allows one to solve the optimization problem
\begin{equation}
  \label{eq:general-optimization-problem}
  \begin{aligned}
    U(X_0) = \min_{\bsx,\bsu} & \sum_{t=0}^T g_t(x_t,u_t) + g_T(x_T) \\
...
\end{equation*}
this occurs at $K=\bar{K}$.}, linear dynamics and no dynamics
\begin{equation}
...
The optimal $K_\infty$ may however not stabilize the system and stabilization happens if and only if all unstable modes are weighted in $Q$:
\begin{theorem}
...
\end{theorem}
Indeed the quantity $||y_t||^2 = x_t^\top CC^\top x_t = x_t^\top Qx_t$
enters the cost function: a feedback law that keeps this quantity finite stabilizes the system.
